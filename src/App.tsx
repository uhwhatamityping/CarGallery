...
import { Menu, Heart, Plus, X, Upload, Loader2, User, Trash2, LogOut, Key, AlertTriangle } from 'lucide-react';
...
import { db, auth, loginWithAIC, logout } from './firebase';
...
function LoginPage({ onClose }: { onClose: () => void }) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const { isNew } = await loginWithAIC(aic);
      if (isNew) {
      ...
      setError(err.message || "Failed to login with AIC.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (showRememberModal) {
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-sm w-full mx-4 text-center animate-in zoom-in">
          <div className="w-16 h-16 bg-yellow-100 text-yellow-600 rounded-full flex items-center justify-center mx-auto mb-4">
            <AlertTriangle className="w-8 h-8" />
          </div>
          <h2 className="text-xl font-bold text-gray-900 mb-2">Important!</h2>
          <p className="text-gray-600 mb-6">
            Please make sure to remember this code! 
            <br/><br/>
            <strong className="text-2xl text-blue-600 tracking-wider font-mono bg-blue-50 px-4 py-2 rounded-lg border border-blue-100">{aic}</strong>
            <br/><br/>
            If you lose it, you will not be able to access your gallery.
          </p>
          <button 
            onClick={onClose}
            className="w-full bg-blue-600 text-white font-medium py-3 px-4 rounded-xl hover:bg-blue-700 transition-all"
          >
            OK, I've remembered it
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900">
      ...
      <div className="relative bg-white p-8 rounded-2xl shadow-2xl max-w-md w-full mx-4 text-center animate-in fade-in zoom-in duration-300">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Account Identification Code</h2>
        <p className="text-gray-500 mb-6 text-sm">
          Enter your AIC to access your gallery. If it doesn't exist, we'll create a new account for you.
        </p>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <input 
            ...
            placeholder="Enter your AIC..."
            className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-center text-lg tracking-widest font-mono uppercase"
          />
          
          <button 
            type="submit"
            disabled={isLoading}
            className="w-full bg-blue-600 text-white font-medium py-3 px-4 rounded-xl hover:bg-blue-700 transition-all flex items-center justify-center disabled:opacity-70"
          >
            {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Continue with AIC'}
          </button>
        </form>
      </div>
    </div>
